...
- **Bump example templates** under `docs/examples/distribution/` on each release so version strings and tag URLs stay in sync with `pyproject.toml` (currently **1.1.1**).
...
| `standalone-binary.sh` | `SPRINGDOCKER_VERSION` env, default `1.1.1` | `.../releases/download/vX.Y.Z/springdocker-linux-amd64.tar.gz` (placeholder until CI ships tarball) |
